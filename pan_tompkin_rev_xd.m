function [qrs_amp_raw,qrs_i_raw,delay]=pan_tompkin_rev_xd(ecg,fs,gr)
...
% to get rid of the baseline wander and muscle noise. (bandpass filtering (5-15Hz) is implemented before the Pan-Tompkin algorithm [XD])
...
THR_SIG = max(ecg_m(1:2*fs))/3; % 0.33 of the max amplitude 
THR_NOISE = mean(ecg_m(1:2*fs))/2; % 0.5 of the mean signal is considered to be noise
SIG_LEV= THR_SIG;
NOISE_LEV = THR_NOISE;


%% Initialize bandpath filter threshold(2 seconds of the bandpass signal)
THR_SIG1 = max(ecg(1:2*fs))/3; % 0.33 of the max amplitude 
THR_NOISE1 = mean(ecg(1:2*fs))/2; %
...
for i = 1 : length(pks)
   ...
   %% locate the corresponding peak in the filtered signal 
    if locs(i)-round(0.150*fs)>= 1 && locs(i)<= length(ecg)
          [y_i x_i] = max(ecg(locs(i)-round(0.150*fs):locs(i)));
       else
          if i == 1
    ...
    if test_m
          if (locs(i) - qrs_i(end)) >= round(1.66*test_m)% it shows a QRS is missed 
              ...
              if pks_temp > THR_NOISE
               ...
               if locs_temp <= length(ecg)
                [y_i_t x_i_t] = max(ecg(locs_temp-round(0.150*fs):locs_temp));
               else
                [y_i_t x_i_t] = max(ecg(locs_temp-round(0.150*fs):end));
               end
...
if gr
figure,az(1)=subplot(311);plot(ecg);title('QRS on Filtered Signal');axis tight;
